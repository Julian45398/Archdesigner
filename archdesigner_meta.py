...
def addConstraints(domain, m2model):
    ser = TextSerializer()
    signalClass = domain.Do(Get(Qry(m2model).Pth(M2PACKAGE.CLASSES).Sel(Pth(M2CLASS.NAME).Equ('Signal')).Idx(0)))
    #signals are note allowed to have the same task as source and destination!
    signalConstraint = ser.SerQry(Pth('src').Neq(Pth('dst')))
    domain.Do(Crt(CONCEPTS.MXCONSTRAINT, 1, [signalClass, signalConstraint]))

    segmentClass = domain.Do(Get(Qry(m2model).Pth(M2PACKAGE.CLASSES).Sel(Pth(M2CLASS.NAME).Equ('RouteSegment')).Idx(0)))
    #Segments must have a connection either to a device or a connection but cannot have both!
